defmodule PartbWeb.StoreLive.FormComponent do
  ...
  def update(%{store: store} = assigns, socket) do
    changeset = StoreSystem.change_store(store)
  ...
  def handle_event("validate", %{"store" => store_params}, socket) do
    changeset =
      ...
      |> StoreSystem.change_store(store_params)
  ...
  defp save_store(socket, :edit, store_params) do
    case StoreSystem.update_store(socket.assigns.store, store_params) do
      {:ok, store} ->
  ...
  defp save_store(socket, :new, store_params) do
    case StoreSystem.create_store(store_params) do
      {:ok, store} ->
        notify_parent({:saved, store})
